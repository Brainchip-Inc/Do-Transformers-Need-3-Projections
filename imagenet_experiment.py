"""
ImageNet-1k ViT-S/16 comparison across four attention conditions (paper design doc,
paper/main.tex): QKV (baseline), Q-K=V, Q=K=V+, and QVV(3). Reuses the same ViT /
SharedProjAttention implementation as vision_tasks.py (MNIST/CIFAR/TinyImageNet), with
a DeiT-style training recipe (RandAugment, Mixup/CutMix, stochastic depth, label
smoothing) added on top, since that recipe is required to train a ViT from scratch
on ImageNet-1k without ImageNet-21k pretraining.
...
CONDITIONS = ["QKV", "Q!=K=V", "(Q=K=V)+", "QVV(3)"]
...
def main():
    ...
    lr = args.base_lr * args.batch_size / 512
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        val_loss, val_top1, val_top5 = evaluate(model, val_loader, device, amp=True)
        ...
        csv_file.flush()
        save_checkpoint(ckpt_path, model, opt, scaler, epoch, best_top1, best_epoch)
